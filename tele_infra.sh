...
#!define PBX_CORE_URL "http://pbx-core.apps.svc.cluster.local:8080"
#!define SIP_DOMAIN "sip.dalaillama.in"
#!define EXTERNAL_IP "YOUR_PUBLIC_IP"
...
__KAMAILIO_TLS_LISTEN__
listen=ws:0.0.0.0:8080
__KAMAILIO_WSS_LISTEN__
...
__KAMAILIO_TLS_MODULE__
loadmodule "jsonrpcs.so"
loadmodule "corex.so"

__KAMAILIO_ENABLE_TLS__
...
route[AUTH_REGISTER] {
    ...
    if (!pv_auth_check("SIP_DOMAIN", "$avp(ha1)", "0")) {
        auth_challenge("SIP_DOMAIN", "0");
        exit;
    }
    consume_credentials();
    if (is_method("REGISTER") && isflagset(FLT_NATS)) {
        add_path_received();
    } else {
        add_path();
    }
...
sed -i "s|YOUR_PUBLIC_IP|${PUBLIC_IP}|g" /etc/kamailio/kamailio.cfg
sed -i "s|http://pbx-core.apps.svc.cluster.local:8080|${PBX_CORE_URL}|g" /etc/kamailio/kamailio.cfg
sed -i "s|sip.dalaillama.in|${SIP_DOMAIN}|g" /etc/kamailio/kamailio.cfg
if [ "${ENABLE_KAMAILIO_TLS}" = "true" ]; then
  sed -i 's|__KAMAILIO_TLS_LISTEN__|listen=tls:0.0.0.0:5061|' /etc/kamailio/kamailio.cfg
  sed -i 's|__KAMAILIO_TLS_MODULE__|loadmodule "tls.so"|' /etc/kamailio/kamailio.cfg
  sed -i 's|__KAMAILIO_ENABLE_TLS__|enable_tls=yes|' /etc/kamailio/kamailio.cfg
...
EOF
  ...
  sed -i 's|__KAMAILIO_TLS_LISTEN__||' /etc/kamailio/kamailio.cfg
  sed -i 's|__KAMAILIO_TLS_MODULE__||' /etc/kamailio/kamailio.cfg
  sed -i 's|__KAMAILIO_ENABLE_TLS__||' /etc/kamailio/kamailio.cfg
fi

if [ "${ENABLE_KAMAILIO_WSS}" = "true" ]; then
  sed -i 's|__KAMAILIO_WSS_LISTEN__|listen=wss:0.0.0.0:7443|' /etc/kamailio/kamailio.cfg
else
  sed -i 's|__KAMAILIO_WSS_LISTEN__||' /etc/kamailio/kamailio.cfg
fi
